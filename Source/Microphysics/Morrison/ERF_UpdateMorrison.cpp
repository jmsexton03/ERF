...
Morrison::Copy_State_to_Micro(const amrex::MultiFab& cons_in)
{
    ...
    amrex::Abort("State_to_Micro not implemented");
    /*
 ...
 */
void
Morrison::Copy_Micro_to_State (amrex::MultiFab& cons)
{
    ...
    for (amrex::MFIter mfi(*mic_fab_vars[MicVar_Morr::qcl]); mfi.isValid(); ++mfi) {
        ...
        amrex::ParallelFor(box, [=] AMREX_GPU_DEVICE (int i, int j, int k) {
            ...
            if (qci(i,j,k) > m_qsmall) {
                ...
                const amrex::Real typical_mass_i = 4.0/3.0 *m_pi* m_rhoi * std::pow(25.0e-6, 3);
            ...
            if (qpr(i,j,k) >= m_qsmall) {
                ...
                lamr(i,j,k) = std::pow(m_pi * m_rhow * nr / qpr(i,j,k), 1.0/3.0);

                // Apply limits to lambda - Lines ~1698-1708
                if (lamr(i,j,k) < m_lamminr) {
                    lamr(i,j,k) = m_lamminr;
                    n0r(i,j,k) = std::pow(lamr(i,j,k), 4) * qpr(i,j,k) / (m_pi * m_rhow);
