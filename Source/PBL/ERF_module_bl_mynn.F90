...
MODULE module_bl_mynn
  ...
  use iso_c_binding !, only: c_double, c_int, c_double, c_bool
...
!!===============================================INTERFACE_TO_C++====================================================
  ...
  interface
    SUBROUTINE  mym_predict_cc (kts,kte,                                &
       ...
       &            xlvcp,xlscp,karman) bind(c)
      import c_int,c_double
    ! these are defs in original
    integer(kind=c_int), intent(in) :: kts,kte
      real(kind=c_double), intent(in)    :: closure
      real(kind=c_double), intent(in)  :: xlvcp, xlscp,karman
      integer(kind=c_int), intent(in) :: bl_mynn_edmf_tke,tke_budget
      real(kind=c_double), dimension(kts:kte), intent(in) :: dz, dfq, el, rho
      real(kind=c_double), dimension(kts:kte), intent(inout) :: pdk, pdt, pdq, pdc
      real(kind=c_double), intent(in)    :: flt, flq, pmz, phh
      real(kind=c_double), intent(in)    :: ust, delt
      real(kind=c_double), dimension(kts:kte), intent(inout) :: qke,tsq, qsq, cov
! WA 8/3/15
      real(kind=c_double), dimension(kts:kte+1), intent(inout) :: s_awqke,s_aw

      !!  TKE budget  (Puhales, 2020, WRF 4.2.1)  << EOB
      real(kind=c_double), dimension(kts:kte), intent(inout) :: qWT1D, qDISS1D
...
SUBROUTINE mynn_tendencies_cc(kts,kte,delt,dz,rho,                &
       ...
       &      diss_heat,s_aw,s_awthl,s_awqt,s_awqv,s_awqc,  &
       &        s_awu,s_awv,                                      &
              &s_awqnc,s_awqni,                      &
       ...
       &        FLAG_QC_I,FLAG_QI_I,FLAG_QNC_I,                       &
       &        FLAG_QNI_I,FLAG_QS_I,                                 &
       &        FLAG_QNWFA_I,FLAG_QNIFA_I,                   &
       &        FLAG_QNBCA_I,FLAG_OZONE_I,                   &
       &        bl_mynn_cloudmix,bl_mynn_mixqt,  &
       &        bl_mynn_edmf_mom,bl_mynn_mixscalars,       &
       &        debug_code,r_d, p608, ep_2,      &
       &        ep_3,tv0,xlv,xlvcp,xlscp) bind(c)
     import c_int,c_double,c_bool
    ! these are defs in original
    integer(kind=c_int), intent(in) :: kts,kte
    ! checked
      integer(kind=c_int), intent(in) :: FLAG_QC_I
      integer(kind=c_int), intent(in) :: FLAG_QI_I,FLAG_QNC_I,     &
               &FLAG_QNI_I,FLAG_QS_I,                        &
               &FLAG_QNWFA_I,FLAG_QNIFA_I,                   &
               &FLAG_QNBCA_I,FLAG_OZONE_I
    integer(kind=c_int), intent(in) :: debug_code
    !checked
    integer(kind=c_int), intent(in) :: bl_mynn_cloudmix,bl_mynn_mixqt,  &
                           bl_mynn_edmf_mom, bl_mynn_mixscalars
    real(kind=c_double), dimension(kts:kte+1), intent(in) :: s_awthl,    &
      &s_aw,s_awqt,s_awqv,s_awqc,s_awu,s_awv,s_awqnc,s_awqni,s_awqnwfa,&
      &s_awqnifa,s_awqnbca,                           &
      &sd_aw,sd_awthl,sd_awqt,sd_awqv,sd_awqc,sd_awu,sd_awv
    real(kind=c_double), intent(in) :: ep_2,ep_3,tv0,xlv,xlvcp,xlscp
! tendencies from mass-flux environmental subsidence and detrainment
    real(kind=c_double), dimension(kts:kte), intent(in) :: sub_thl,sub_sqv,   &
         &sub_u,sub_v,det_thl,det_sqv,det_sqc,det_u,det_v
    real(kind=c_double), dimension(kts:kte), intent(in) ::u,v,th,tk,rho,p,exner,dz,   &
    & qv,tcd,qcd,diss_heat,qc,qi,qs,qnc,qni
    real(kind=c_double), dimension(kts:kte), intent(inout) :: thl,sqw,sqv,sqc,&
         &dfm,dfh,sqi,sqs,qnwfa,qnifa,qnbca,ozone
    real(kind=c_double), dimension(kts:kte), intent(inout) ::Du,Dv,Dth,dqv,  &
         &dqc,dqi,dqs,dqni,dqnc,dqnwfa,dqnifa,dqnbca,dozone
    real(kind=c_double), intent(in) :: flt,flq,flqv,flqc,uoce,voce
    real(kind=c_double), intent(in) :: ust,delt,psfc,wspd
    real(kind=c_double), intent(in) :: r_d, p608
  END SUBROUTINE mynn_tendencies_cc
    ...
    real(kind=c_double), dimension(kts:kte), intent(in)    :: dfh,dz,tcd,qcd
    real(kind=c_double), dimension(kts:kte), intent(inout) :: rho
    real(kind=c_double), intent(in)    :: flt
    real(kind=c_double), intent(in)    :: delt,pblh
    integer(kind=c_int), intent(in) :: nchem, kdvel, ndvel
    real(kind=c_double), dimension( kts:kte+1), intent(in) :: s_aw
    real(kind=c_double), dimension( kts:kte, nchem ), intent(inout) :: chem1
    real(kind=c_double), dimension( kts:kte+1,nchem), intent(in) :: s_awchem
    real(kind=c_double), dimension( ndvel ), intent(in) :: vd1
    real(kind=c_double), intent(in) :: emis_ant_no,frp
  ...
  !! applying corresponding input tendencies and corrective tendencies.
    ...
    real(kind=c_double), intent(in)     :: delt
    real(kind=c_double), dimension(kte), intent(in)     :: dp, exner
    real(kind=c_double), dimension(kte), intent(inout)  :: qv, qc, qi, qs, th
    real(kind=c_double), dimension(kte), intent(inout)  :: dqv, dqc, dqi, dqs, dth
...
!-------------------------------------------------------------------
    ...
    real(kind=c_double), intent(in), value :: ep_2, ep_3,xlv, r_d, xlvcp, &
                  p608, tv0,cpv,r_v,cice,cliq,cp,xls,rcp
    real(kind=c_double), intent(in)      :: HFX1,rmo,xland
    real(kind=c_double), intent(in)      :: dx,pblh1
    real(kind=c_double), dimension(kts:kte), intent(in) :: dz
    real(kind=c_double), dimension(kts:kte+1), intent(in) :: zw
    real(kind=c_double), dimension(kts:kte), intent(in) :: p,exner,thl,qw,   &
         &qv,qc,qi,qs,tsq,qsq,cov,th,rstoch_col
    real(kind=c_double), dimension(kts:kte), intent(inout) :: vt,vq,sgm
    real(kind=c_double), dimension(kts:kte) :: alp,a,bet,b,ql,q1,RH
    real(kind=c_double), dimension(kts:kte), intent(out) :: qc_bl1D,qi_bl1D, &
         &cldfra_bl1D
    real(kind=c_double), dimension(kts:kte), intent(in) :: Sh,el
 ...
 SUBROUTINE topdown_cloudrad_cc(kts,kte,                         &
    ...
    import c_int, c_double
    real(kind=c_double), intent(inout)                    :: maxKHtopdown
    real(kind=c_double),dimension(kts:kte), intent(inout) :: KHtopdown,TKEprodTD
    integer(kind=c_int),intent(in)        :: kte,kts
    real(kind=c_double), dimension(kts:kte), intent(in)   :: dz1,sqc,sqi,sqw,&
          thl,th1,ex1,p1,rho1,thetav,cldfra_bl1D
    real(kind=c_double), dimension(kts:kte), intent(in)   :: rthraten
    real(kind=c_double), dimension(kts:kte+1), intent(in) :: zw
    real(kind=c_double), intent(in)                       :: pblh,fltv
    real(kind=c_double), intent(in)                       :: xland
  ...
  SUBROUTINE DDMF_JPL_cc(kts,kte,dt,zw,dz,p,         &
              ...
              & p1000mb,rcp,xlvcp,cp,rvovrd          ) bind(c)
        import c_int, c_double
        integer(kind=c_int), intent(in) :: kts,kte,kpbl
        real(kind=c_double), dimension(kts:kte), intent(in) ::         &
            u,v,th,thl,tk,qt,qv,qc,thv,p,rho,exner,dz
        real(kind=c_double), dimension(kts:kte), intent(in) :: rthraten
        real(kind=c_double), dimension(kts:kte), intent(inout) :: edmf_a_dd,&
                edmf_w_dd,edmf_thl_dd,edmf_ent_dd,edmf_qc_dd,edmf_qt_dd
        real(kind=c_double), dimension(kts:kte+1), intent(inout) ::sd_aw, &
                sd_awthl,sd_awqt,sd_awqv,sd_awqc,sd_awu,sd_awv,sd_awqke
        real(kind=c_double), dimension(kts:kte), intent(in) :: qc_bl1d,cldfra_bl1d
        ! zw .. heights of the downdraft levels (edges of boxes)
        real(kind=c_double), dimension(kts:kte+1), intent(in) :: zw
        real(kind=c_double), intent(in)  :: dt,ust,pblh,wthl,wqt
        real(kind=c_double), intent(in)  :: svp1,grav,onethird,p1000mb,rcp,xlvcp,cp,rvovrd
    ...
    SUBROUTINE retrieve_exchange_coeffs_cc(kts,kte,&
       &dfm,dfh,dz,K_m,K_h) bind(c)
        ...
        integer(kind=c_int) , intent(in) :: kts,kte
        real(kind=c_double), dimension(KtS:KtE), intent(in) :: dz,dfm,dfh
        real(kind=c_double), dimension(KtS:KtE), intent(out) :: K_m, K_h
    END SUBROUTINE retrieve_exchange_coeffs_cc

   SUBROUTINE get_pblh_cc(kts,kte,zi,thetav1D, &
      &qke1D,zw1D,dz1D,landsea,kzi) bind(c)
      import c_double, c_int
      integer(kind=c_int),intent(in) :: kts,kte
      real(kind=c_double), intent(in) :: landsea
      real(kind=c_double), intent(inout) :: zi
      integer(kind=c_int), intent(inout) :: kzi
      real(kind=c_double),dimension(kts:kte),intent(in)::thetav1D,qke1D,dz1D
      real(kind=c_double),dimension(kts:kte+1), intent(in) :: zw1D
    END SUBROUTINE get_pblh_cc

    SUBROUTINE scale_aware_cc(dx,PBL1,Psig_bl,Psig_shcu) bind(c)
        import c_double
        real(kind=c_double), intent(in)  :: dx,pbl1
        real(kind=c_double), intent(out) :: Psig_bl,Psig_shcu
    END SUBROUTINE scale_aware_cc

  SUBROUTINE dmp_mf_cc(                            &
            ...
            ! inputs for stochastic perturbations
                 & spp_pbl,rstoch_col,grav,gtr, &
                 & p608   ) bind(c)
         import c_double, c_int
  ! inputs:
     integer(kind=c_int), intent(in) :: kts,kte,kpbl,momentum_opt,tke_opt,scalar_opt
! Stochastic
     integer(kind=c_int),intent(in)  :: spp_pbl,mix_chem,nchem
     real(kind=c_double), dimension(kts:kte)  :: rstoch_col

     real(kind=c_double),dimension(kts:kte), intent(in) ::                 &
          &U,V,W,TH,THL,TK,QT,QV,QC,                                   &
          &dz,THV,P,rho,qke,qnc,qni,qnwfa,qnifa,qnbca
     real(kind=c_double),dimension(kts:kte+1), intent(in) :: zw    !height at full-sigma
     real(kind=c_double), intent(in) :: flt,fltv,flq,flqv,Psig_shcu,       &
          &landsea,ts,dx,dt,ust,pblh
     real(kind=c_double), intent(in), value :: grav, gtr, p608
     real(kind=c_double),dimension(kts:kte), intent(out) :: edmf_a,edmf_w, &
          & edmf_qt,edmf_thl,edmf_ent,edmf_qc
     integer(kind=c_int), intent(out) :: ktop
     real(kind=c_double), intent(out) :: maxmf,ztop,maxwidth
     real(kind=c_double),dimension(kts:kte), intent(inout) ::            &
          &qc_bl1d,cldfra_bl1d,qc_bl1d_old,cldfra_bl1d_old
     real(kind=c_double),dimension(kts:kte+1), intent(in) :: s_aw,       & !sum ai*rho*wis_awphi
          &s_awthl,s_awqt,s_awqv,s_awqc,s_awqnc,s_awqni,s_awqke,        &
          &s_awqnwfa,s_awqnifa,s_awqnbca,s_awu,s_awv
     real(kind=c_double),dimension(kts:kte+1,nchem), intent(in) :: s_awchem
     real(kind=c_double), dimension(kts:kte,nchem), intent(in) :: chem1

 END SUBROUTINE dmp_mf_cc

  SUBROUTINE  mym_turbulence_cc (                             &
    &            kts,kte,                                     &
    &            xland,closure,                               &
    &            dz, dx, zw,                                  &
    ...
    &            TKEprodTD,                                   &
    &            spp_pbl,rstoch_col,                          &
    &            debug_code, gtr, tv0) bind(c)
    import c_double, c_int
    integer(kind=c_int), intent(in)   :: kts,kte
    integer(kind=c_int), intent(in)   :: debug_code
    integer(kind=c_int), intent(in)   :: bl_mynn_mixlength,tke_budget
    real(kind=c_double), intent(in)    :: closure, gtr, tv0
    real(kind=c_double), dimension(kts:kte),   intent(in) :: dz
    real(kind=c_double), dimension(kts:kte),   intent(out):: sm,sh
    real(kind=c_double), dimension(kts:kte+1), intent(in) :: zw
    real(kind=c_double), intent(in)       :: rmo,flt,fltv,flq,                 &
         &Psig_bl,Psig_shcu,xland,dx,zi
    real(kind=c_double), dimension(kts:kte), intent(in) :: u,v,thl,thetav,qw,  &
         &ql,vt,vq,qke,tsq,qsq,cov,cldfra_bl1D,edmf_w1,edmf_a1,            &
         &TKEprodTD, theta
    real(kind=c_double), dimension(kts:kte), intent(out) :: dfm,dfh,dfq,       &
         &pdk,pdt,pdq,pdc,tcd,qcd,el
    real(kind=c_double), dimension(kts:kte), intent(inout) ::                  &
         qWT1D,qSHEAR1D,qBUOY1D,qDISS1D
    real(kind=c_double), dimension(kts:kte),intent(in)     :: rstoch_col
    integer(kind=c_int),         intent(in)                   ::    spp_pbl

  END SUBROUTINE mym_turbulence_cc

 SUBROUTINE  mym_initialize_cc (                                &
       &            kts,kte,xland,                            &
       &            dz, dx, zw,                               &
       &            u, v, thl, qw,                            &
       ...
       &            edmf_w1,edmf_a1,                          &
       &            INITIALIZE_QKE,                           &
       &            spp_pbl,rstoch_col, karman, tv0, gtr) bind(c)
    import c_double, c_int
    integer(kind=c_int), intent(in)           :: kts,kte
    integer(kind=c_int), intent(in)           :: bl_mynn_mixlength
    integer(kind=c_int), intent(in)           :: INITIALIZE_QKE, spp_pbl
    real(kind=c_double), intent(in)   :: rmo, Psig_bl, xland
    real(kind=c_double), intent(in)   :: dx, ust, zi, karman, tv0, gtr
    real(kind=c_double), dimension(kts:kte),   intent(in) :: dz
    real(kind=c_double), dimension(kts:kte+1), intent(in) :: zw
    real(kind=c_double), dimension(kts:kte),   intent(in) :: u,v,thl,&
         &qw,cldfra_bl1D,edmf_w1,edmf_a1
    real(kind=c_double), dimension(kts:kte),   intent(out) :: tsq,qsq,cov
    real(kind=c_double), dimension(kts:kte),   intent(inout) :: el,qke
    real(kind=c_double), dimension(kts:kte), intent(in) :: theta,thetav,&
    &    sh,sm, rstoch_col
  END SUBROUTINE mym_initialize_cc
...
CONTAINS
  SUBROUTINE mynn_bl_driver_test(initflag) &
    BIND(C, name='mynn_bl_driver_test')
...
#define WRF_TEST_CC_1 1
#define WRF_TEST_CC_2
#define WRF_TEST_CC
#define WRF_TEST_CC_FLOAT
#undef WRF_TEST_CC_FLOAT
#undef WRF_TEST_CC_1
...
!local vars
    ...
    integer, dimension(1:9)          :: flags_integer
    logical, dimension(1:9)          :: flags_logical

    integer :: FLAG_QC_I,FLAG_QI_I,FLAG_QNC_I,       &
       &FLAG_QNI_I,FLAG_QS_I,               &
       &FLAG_QNWFA_I,FLAG_QNIFA_I,          &
       &FLAG_QNBCA_I,FLAG_OZONE_I
       real(kind_phys) ::  cice_cc, cliq_cc, cp_cc, cpv_cc, ep_2_cc, &
       &karman_cc, p1000mb_cc, p608_cc, r_d_cc, r_v_cc, &
       &rcp_cc, rvovrd_cc, svp1_cc, xls_cc, xlv_cc
...
!#ifdef WRF_TEST_CC_2
!      CLOSE(10)
!      OPEN(10, file='output_cc.txt', status='replace')
!!#elif defined(WRF_TEST_EQ)
!!      CLOSE(10)
!!      OPEN(10, file='cc_pblh_scaleaware_cond_predict.txt', status='replace')
!#else
!      CLOSE(10)
!      OPEN(10, file='output_F.txt', status='replace')
!#endif
!      flags_logical = (/ FLAG_QI,FLAG_QNI,FLAG_QC,FLAG_QNC,FLAG_QNWFA,FLAG_QNIFA,FLAG_QNBCA, FLAG_OZONE,FLAG_QS /)

!      do i = 1, size(flags_logical)
!         if(flags_logical(i) .eqv. .true.) then
!            flags_integer(i)=1
!         else
!            flags_integer(i)=0
!         endif
!      end do
      ...
      FLAG_QNI_I=FLAG_QNI
      FLAG_QS_I=FLAG_QS_I
      FLAG_QNWFA_I=FLAG_QNWFA
      FLAG_QNIFA_I=FLAG_QNIFA
      FLAG_QNBCA_I=FLAG_QNBCA
      FLAG_OZONE_I=FLAG_OZONE
...
!! at every i and j point, corresponding to the x- and y- directions, respectively.
    IF (initflag > 0 .and. .not.restart) THEN
       ...
       DO i=ITS,ITF !this is where we go from 2d to 1d
          if (FLAG_QI ) then
             sqi2(kts:kte)=sqi3D(i,kts:kte)
          else
             sqi2 = 0.0
          endif
          if (FLAG_QS ) then
             sqs2(kts:kte)=sqs3D(i,kts:kte)
          else
             sqs2 = 0.0
          endif
          if (icloud_bl > 0) then
...
!#ifdef WRF_TEST_CC
#ifdef WRF_TEST_EQ
             CALL get_pblh_cc(KTS_cc,KTE_cc,PBLH(i),thetav,Qke1,zw,dz1,xland(i),KPBL_cc(i))
             KPBL(i)=KPBL_cc(i)+1
#else
             CALL GET_PBLH(KTS,KTE,PBLH(i),thetav,&
               &  Qke1,zw,dz1,xland(i),KPBL(i))
#endif
!             write(10,*),KTS,KTE,PBLH(i),thetav,&
!             &  Qke1,zw,dz1,xland(i),KPBL(i)
!             write(10,*),SIZEOF(KTS),SIZEOF(KTE),SIZEOF(PBLH(i)),SIZEOF(thetav),&
!             &  SIZEOF(Qke1),SIZEOF(zw),SIZEOF(dz1),SIZEOF(xland(i)),SIZEOF(KPBL(i))
!             STOP
...
!#ifdef WRF_TEST_CC
             ...
             ELSE
                ...
                Psig_shcu(i)=1.0
             ENDIF
!             write(10,*),dx(i),PBLH(i),Psig_bl(i),Psig_shcu(i)
!             STOP
...
!! within mym_initialize(): mym_level2() and mym_length().
             if (INITIALIZE_QKE .eqv. .true.) then
                 INITIALIZE_QKE_I = 1
             else
                 INITIALIZE_QKE_I=0
              endif
!! differences like 2e-9 in qke and identical in T, U, V, W
!     #ifdef WRF_TEST_CC_FLOAT
#ifdef WRF_TEST_CC_1
             CALL mym_initialize_cc (             &
                  &kts_cc,kte_cc,xland(i),              &
                  &dz1, dx(i), zw,                &
                  &u1, v1, thl, sqv2,             &
             ...
             CALL mym_initialize (                &
                  ...
                  &spp_pbl,rstoch_col             )
#endif
!write(*,'(12(F25.16))'),pr ,g1,             &  ! NN2009 = 0.235
!       &b1  ,              &
!       &b2  ,              &  ! CKmod     NN2009
!       &c2  ,              &  ! 0.729, & !0.75, &
!       &c3  ,              &  ! 0.340, & !0.352, &
...
!                  &dz1, dx(i), zw,                &
!                  &u1, v1, thl, sqv2,              &
!                  &PBLH(i), th1, "thetav", thetav, "sh", sh,"sm", sm,  &
!                  &"ust",ust(i), rmol(i),               &
!                  &el, Qke1, Tsq1, Qsq1, Cov1,    &
!                  &Psig_bl(i), cldfra_bl1D,       &
!                  &bl_mynn_mixlength,             &
...
!! performed at every time step are ready for execution.
    ...
    DO i=ITS,ITF
       ...
       sqv2(kts:kte)    =sqv3D(i,kts:kte) !/(1.+qv(i,kts:kte))
       sqc(kts:kte)    =sqc3D(i,kts:kte) !/(1.+qv(i,kts:kte))
       qv1(kts:kte)    =sqv2(kts:kte)/(1.-sqv2(kts:kte))
       qc1(kts:kte)    =sqc(kts:kte)/(1.-sqv2(kts:kte))
       qi1(kts:kte)    =sqi2(kts:kte)/(1.-sqv2(kts:kte))
       qs1(kts:kte)    =sqs2(kts:kte)/(1.-sqv2(kts:kte))
       dqc1(kts:kte)   =0.0
       dqi1(kts:kte)   =0.0
       dqs1(kts:kte)   =0.0
       dqni1(kts:kte)  =0.0
...
!             write(10,'(4(F25.16))'),dx(i),PBLH(i),Psig_bl(i),Psig_shcu(i)
!             STOP
...
!! differences like 4e-9 in qke and identical in T, U, V, W
!#ifdef WRF_TEST_CC_1
#ifdef WRF_TEST_EQ
       call mym_condensation_cc (kts_cc,kte_cc,          &
            &dx(i),dz1,zw,xland(i),                      &
            &thl,sqw,sqv2,sqc,sqi2,sqs2,                 &
            ...
            &spp_pbl,rstoch_col,                         &
            &ep_2_cc,ep_3,xlv_cc,r_d_cc,xlvcp,                    &
            &p608_cc,tv0,r_v_cc,cice_cc,cliq_cc,                     &
            &cpv_cc,cp_cc,xls_cc,rcp_cc)
#else
...
!! \p bl_mynn_topdown =1.
       ...
       if (bl_mynn_topdown.eq.1) then
          call topdown_cloudrad_cc(kts_cc,kte_cc,dz1,zw,fltv,     &
               &xland(i),kpbl_cc(i),PBLH(i),                &
               &sqc,sqi2,sqw,thl,th1,ex1,p1,rho1,thetav,  &
...
!               &maxKHtopdown(i),KHtopdown,TKEprodTD
!               STOP
!! matches when bl_mynn_edmf=1 but currently not updating sub_ variables in cc
#ifdef WRF_TEST_CC_1
       kpbl_cc(i) = kpbl(i) - 1
       if (bl_mynn_edmf > 0) then
          !PRINT*,"Calling DMP Mass-Flux: i= ",i
          if(mix_chem .eqv. .true.) then
              mix_chem_i=1
          else
              mix_chem_i=0
          endif
          call dmp_mf_cc(                                   &
               &kts_cc,kte_cc,delt,zw,dz1,p1,rho1,             &
               &bl_mynn_edmf_mom,                        &
               &bl_mynn_edmf_tke,                        &
               &bl_mynn_mixscalars,                      &
            ...
            ! chem/smoke mixing
               ...
               &spp_pbl,rstoch_col, grav, gtr, p608      )
       endif
#else
       if (bl_mynn_edmf > 0) then
          !PRINT*,"Calling DMP Mass-Flux: i= ",i
...
#endif
!       write(10,'(7(I6))'),kts,kte,               &
!               &bl_mynn_edmf_mom,                        &
!               &bl_mynn_edmf_tke,                        &
!               &bl_mynn_mixscalars,nchem,ktop_plume(i),kpbl(i),spp_pbl
!               write(10,*), mix_chem,                    &
!               &FLAG_QC,FLAG_QI,                         &
!               &FLAG_QNC,FLAG_QNI,                       &
!               &FLAG_QNWFA,FLAG_QNIFA,FLAG_QNBCA
...
!#ifdef WRF_TEST_CC_1
       ...
       if (bl_mynn_edmf_dd == 1) then
          call DDMF_JPL_cc(kts_cc,kte_cc,delt,zw,dz1,p1,       &
               &u1,v1,th1,thl,thetav,tk1,                &
               &sqw,sqv2,sqc,rho1,ex1,                    &
               &ust(i),flt,flq,                          &
               &PBLH(i),KPBL_cc(i),                         &
               ...
               &rthraten(i,:),svp1_cc,grav,onethird,        &
               &p1000mb_cc,rcp_cc, xlvcp,cp_cc,rvovrd_cc             )
       endif
       ...
       delt2 = delt !*0.5    !only works if topdown=0
#endif
!       write(10,*),kts,kte,delt,zw,dz1,p1,          &
!               &u1,v1,th1,thl,thetav,tk1,                &
!               &sqw,sqv2,sqc,rho1,ex1,                    &
!               &ust(i),flt,flq,                          &
!               &PBLH(i),KPBL(i),                         &
...
!! differences like 4e-9 in qke and identical in T, U, V, W
!     #ifdef WRF_TEST_CC_FLOAT
!     too many of the differences in the arguments are things that should be nans.
! some possibly significant differences in the arguments remain
#ifdef WRF_TEST_CC_FLOAT
       call mym_turbulence_cc(                           &
               &kts_cc,kte_cc,xland(i),closure,          &
               &dz1, DX(i), zw,                          &
               ...
               &edmf_w1,edmf_a1,                         &
               &TKEprodTD,                               &
               &spp_pbl,rstoch_col,debug_code_i,         &
               &gtr, tv0)
#else
       call mym_turbulence(                              &
...
!               &Psig_bl(i),Psig_shcu(i),                 &
!               &cldfra_bl1D
!               write(10,*), "mixlength",bl_mynn_mixlength
!               write(10,'(34(F25.16))'), &
!               &edmf_w1,edmf_a1,                         &
!               &TKEprodTD
!               write(10,*),                               &
!               &spp_pbl,rstoch_col
!STOP
!! differences like 2e-8 in qke and identical in T, U, V, W
!#ifdef WRF_TEST_CC_FLOAT
!#ifdef WRF_TEST_CC_1
#ifdef WRF_TEST_EQ
...
!! for the following time step.
       call mym_predict_cc(kts_cc,kte_cc,closure,                 &
               &delt2, dz1,                              &
...
!! for the following time step.
       call mym_predict(kts,kte,closure,                 &
               ...
               &qWT1, qDISS1, tke_budget                 )
#endif
!write(10,*),kts,kte,closure,                 &
!               &delt2, dz1,                              &
!               &ust(i), flt, flq, pmz, phh,              &
!               &el, dfq, rho1,"pdk", pdk, pdt,"pdq", pdq, pdc,       &
!               &"Qke1",Qke1, "Tsq1",Tsq1, Qsq1, Cov1,                  &
!               &s_aw1, s_awqke1, bl_mynn_edmf_tke,       &
!               &qWT1, qDISS1, tke_budget
!               STOP
       if (dheat_opt > 0) then
...
!! binary diff identical
!! matches exactly
!#ifdef WRF_TEST_CC_1
#ifdef WRF_TEST_EQ
!>  - Call mynn_tendencies() to solve for tendencies of
!     ! \f$U, V, \theta, q_{v}, q_{c}, and q_{i}\f$.
       call mynn_tendencies_cc(kts_cc,kte_cc,delt, dz1, rho1,    &
       &u1,v1,th1,tk1,qv1,qc1,qi1,kzero,qnc1,qni1,&
       &ps(i),p1,ex1,thl,sqv2,sqc,sqi2,kzero,sqw,&
       &qnwfa1, qnifa1, qnbca1, ozone1,          &
       &ust(i),flt,   &
               &flq,flqv,flqc,wspd(i),uoce(i),voce(i),tcd, &
               &qcd,dfm,dfh,Du1,Dv1,Dth1,Dqv1,            &
               ...
               &sub_sqv,sub_u,sub_v, &
               &det_thl,det_sqv,det_sqc,det_u,det_v,       &
               &FLAG_QC_I,FLAG_QC_I,FLAG_QNC_I,                &
               &FLAG_QNI_I,FLAG_QS_I,                        &
               &FLAG_QNWFA_I,FLAG_QNIFA_I,                   &
               &FLAG_QNBCA_I,FLAG_OZONE_I,                   &
               &bl_mynn_cloudmix,bl_mynn_mixqt,  &
               &bl_mynn_edmf_mom,bl_mynn_mixscalars,       &
               &debug_code_i,r_d_cc,p608_cc,ep_2_cc,ep_3,tv0,xlv_cc,   &
               &xlvcp,xlscp)
#else
!>  - Call mynn_tendencies() to solve for tendencies of
!! \f$U, V, \theta, q_{v}, q_{c}, and q_{i}\f$.
       call mynn_tendencies(kts,kte,i,                   &
               ...
               &ps(i), p1, ex1, thl,                     &
               &sqv2, sqc, sqi2, kzero, sqw,               & !kzero replaces sqs - not mixing snow
               &qnwfa1, qnifa1, qnbca1, ozone1,          &
...
!               &cldfra_bl1d
!      STOP
!! matches arguments first call, mostly zeros
#ifdef WRF_TEST_EQ
       if ( mix_chem ) then
          if (enh_mix) then
                  enh_mix_in=1
          ...
          if ( rrfs_sd ) then
             call mynn_mix_chem_cc(kts_cc,kte_cc,i,               &
                  &delt, dz1, pblh(i),                   &
                  &nchem, kdvel, ndvel,                  &
                  &chem1, vd1,                           &
                  &rho1,flt,                             &
...
#endif
!       write(10,'(7(I6))'),kts,kte,i,nchem,kdvel,ndvel
!       write(10,*),rrfs_sd,smoke_dbg,enh_mix
...
!                  &tcd, qcd,                             &
!                  &dfh,                                  &
!                  &s_aw1,s_awchem1,chem3d
!      STOP
!! matches output
!#ifdef WRF_TEST_CC_1
#ifdef WRF_TEST_EQ
       call retrieve_exchange_coeffs_cc(kts_cc,kte_cc,            &
...
!
    ...
    shc = 3.0*a2*( g1+g2 )
!
!!!      write(*,'(A3, 5(F25.16))'),"g1 ",g1,g2,rfc,rf1,rf2
!!!      write(*,'(A3, 7(F25.16))'),"g2 ",g2,b2,b1,( 1.0-c3 ) ,2.0*a1,b1,( 3.0-2.0*c2 )
    ri1 = 0.5/smc
    ri2 = rf1*smc
...
!
       gm (k) =  duz
       gh (k) = -dtq*gtr  ! gtr == grav / tref (kind_phys)
!
...
!!!       write(*,'(A6, 3(F25.16))'),"rf    ",rf2,ri2,ri3,smc
!!!       write(*,*) ri4
       sh (k) = shc*( rfc-rf )/( 1.0-rf )
       sm (k) = smc*( rf1-rf )/( rf2-rf ) * sh(k)
!!!       write(*,'(A6, 5(F25.16))'),"sm[k] ",sm(k),shc,rfc,rf,1.0
!!!       write(*,'(A6, 5(F25.16))'),"sh[k] ",sh(k),smc,rf1,rf,rf2
...
!
       ...
       ELSE
        ...
        !  sm_pbl = q3sq*a1*( e3-3.0*c1*e4       )/eden
          ...
          !!sh(k) = q3sq*a2*( e2+3.0*c1*e5c*gmel )/eden
          sh(k) = q3sq*(a2*a2fac)*( e2+3.0*c1*e5c*gmel )/eden
!          print*,"sh[k]",k,sh(k),q3sq,a2,a2fac,e2,c1,e5c,gmel,eden
        !  sm(k) = Prnum*sh(k)
...
!       qke(k)=max(d(k-kts+1), qkemin)
       ...
       qke(k)=min(qke(k), 150.)
!     write(*,'(7(F25.16))'),qke(k),x(k),qkemin,a(k),b(k),c(k),d(k)
!       write(*,'(6(F30.25))'),qke(k),a(k),b(k),c(k),d(k),1.0*k
    ENDDO
!     !  TKE budget  (Puhales, 2020, WRF 4.2.1)  << EOB
    IF (tke_budget .eq. 1) THEN
...
!!!       write(*,'(A6, 5(F25.16))'),"rac   ",rac,alp(k),cldfra_bl1d(k),qll,eq1,q2p,qt,p608,pt
!!!       write(*,'(A6, 5(F25.16))'),"vt[k] ",vt(k),qt,rac,bet(k),dqsl
!!!       write(*,'(A6, 5(F25.16))'),"vq[k] ",vq(k),p608,pt,tv0,rac
...
!!!       write(*,'(A6, 5(F25.16))'),"vt[k] ",vt(k),cfmax,beta,bb,Fng
!!!       write(*,'(A6, 5(F25.16))'),"vq[k] ",vq(k),alpha, cfmax, beta,a(k),tv0
        enddo
...
!      n - number of unknowns (levels)
        ...
        cp(1) = c(1)/b(1)
        dp(1) = d(1)/b(1)
!        write(*,'(4(F30.25))'),c(1),c(1)/b(1),b(1),a(1)
        ! solve for vectors c-prime and d-prime
        do i = 2,n
           m = b(i)-cp(i-1)*a(i)
           cp(i) = c(i)/m
           dp(i) = (d(i)-dp(i-1)*a(i))/m
!           write(*,'(4(F30.25))'),c(i),m,b(i),a(i)
        enddo
        ! initialize x
        x(n) = dp(n)
        ...
        do i = n-1, 1, -1
           x(i) = dp(i)-cp(i)*x(i+1)
!           write(*,'(4(F30.25))'),x(i),dp(i),cp(i),x(i+1)
        end do
